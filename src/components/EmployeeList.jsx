...
export const EmployeeList = () => {
  const { employees, mail, removeEmployee } = useContext(GlobalContext);
  return (
    <React.Fragment>
      {employees.length > 0 ? (
        <React.Fragment>
      ...
      {mail?.map((m) => (
        <div className="flex items-center bg-gray-100 mb-10 shadow" key={m.id}>
          <div className="flex-auto text-left px-4 py-2 m-2">
            <p className="text-gray-900 leading-none">{m.email}</p>
            <p className="text-gray-600">{m.addrtype}</p>
          </div>
        </div>
      ))}
    </React.Fragment>
